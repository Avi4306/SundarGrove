...
export default function Homepage() {
  ...
  return (
    <div 
      className="relative flex flex-col items-center justify-center min-h-screen bg-[url('./src/assets/SG-1.jpg')] bg-cover bg-center overflow-hidden cursor-crosshair"
    ...
    >
      ...
      {leaves.map(leaf => (
        <div
          key={leaf.id}
          className="absolute w-4 h-7 opacity-15 pointer-events-none bg-gradient-to-br from-emerald-600 to-green-500"
          style={{
            left: `${leaf.x}%`,
            top: `${leaf.y}%`,
      ...
      {ripples.map(ripple => (
        ...
        >
          <div className="w-4 h-4  border-2 border-cyan-400 rounded-full animate-ping opacity-60"></div>
          <div className="absolute inset-0 w-8 h-8 border border-emerald-400 rounded-full animate-ping opacity-30" style={{animationDelay: '0.2s'}}></div>
        </div>
      ))}
      ...
      <div className="relative z-20 mb-12">
        <div className="relative group">
          <img 
            src={logo} 
            alt="SundarGrove" 
            className="h-60 drop-shadow-xl filter brightness-110 group-hover:brightness-125 transition-all duration-500" 
          />
        </div>
      </div>
      
      {/* Compact Content Card */}
      <div className="relative z-20 mt-[-20px] bg-[rgba(255,255,255,0.4)] backdrop-blur-md border border-emerald-200/60 rounded-3xl shadow-xl p-8 w-full max-w-2xl mx-6 transform hover:scale-[1.05] hover:shadow-2xl transition-all duration-200">
        
        {/* Welcome Text */}
        <h1 className="text-4xl md:text-5xl font-black mb-6 bg-gradient-to-r from-emerald-800 to-green-700 bg-clip-text text-transparent text-center leading-tight">
